...
class LocationJsonParser : LocationParser {
    private fun parseLocationFile(path: String): List<LocationRecipe> = jacksonObjectMapper().readValue(this::class.getResourceAsStream(path))
    private fun parseLocationNodeFile(path: String): List<LocationNode> = jacksonObjectMapper().readValue(this::class.getResourceAsStream(path))
