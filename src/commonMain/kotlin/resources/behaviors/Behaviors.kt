...
class BaseBehaviors : BehaviorResource {
    override val values = listOf<Behavior<*>>(
            Behavior("Add on Eat", ConditionalEvents(EatFoodEvent::class, createEvents = { _, params ->
                listOf(SpawnItemEvent(params["resultItemName"] ?: "Apple", params["count"]?.toInt() ?: 1, GameState.player))
            })),

            Behavior("Chop Tree", ConditionalEvents(PropertyStatMinnedEvent::class, { event, _ ->
            ...
            Behavior("Climbable", ConditionalEvents(InteractEvent::class, createEvents = { event, _ ->
                listOf(CommandEvent("climb ${event.target.name}"))
            })),

            Behavior("Burn to Ash", ConditionalEvents(PropertyStatMinnedEvent::class, { event, _ ->
            ...
            Behavior("Burn Out", ConditionalEvents(PropertyStatMinnedEvent::class, { event, _ ->
            ...
            Behavior("Slash Harvest", ConditionalEvents(UseEvent::class, { event, _ ->
            ...
            Behavior("Restrict Destination", ConditionalEvents(InteractEvent::class, createEvents = { event, params ->
            ...
            Behavior("Mill", ConditionalEvents(ItemPickedUpEvent::class, { event, params ->
            ...
            Behavior("Learn Recipe", ConditionalEvents(InteractEvent::class, createEvents = { event, params ->
